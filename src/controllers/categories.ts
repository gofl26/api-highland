...
export const createCategory = (async (
  ...
  try {
    const { query, values } = buildCreateQuery('categories', req.body)
...
export const getCategory = async (req: Request, res: Response, next: NextFunction) => {
  const client = await pool.connect()
  try {
    const getQueryResponse = await buildGetQuery(req, next, 'categories')
    if (!getQueryResponse) throw new HttpError('Query generation failed', 500)
    const getQueryTotalResponse = await buildGetTotalQuery(req, next, 'categories')
...
export const updateCategory = (async (
  ...
  try {
    ...
    if (body.categoryName !== undefined) {
      Object.assign(update.data, { categoryName: body.categoryName })
      Object.assign(update.fieldMap, { categoryName: 'category_name' })
    }
