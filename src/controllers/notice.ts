...
export const createNotice = (async (
  ...
  try {
    ...
    const { query, values } = buildCreateQuery('notice', req.body)
...
export const getNotice = async (req: Request, res: Response, next: NextFunction) => {
  const client = await pool.connect()
  try {
    const getQueryResponse = await buildGetQuery(req, next, 'notice')
    if (!getQueryResponse) throw new HttpError('Query generation failed', 500)
    const getQueryTotalResponse = await buildGetTotalQuery(req, next, 'notice')
...
export const updateNotice = (async (
  ...
  try {
    ...
    if (body.noticeCategory !== undefined) {
      Object.assign(update.data, { noticeCategory: body.noticeCategory })
      Object.assign(update.fieldMap, { noticeCategory: 'notice_category' })
    }
    if (body.noticeTitle !== undefined) {
      Object.assign(update.data, { noticeTitle: body.noticeTitle })
      Object.assign(update.fieldMap, { noticeTitle: 'notice_title' })
    }
    if (body.noticeActive !== undefined) {
      Object.assign(update.data, { noticeActive: body.noticeActive })
      Object.assign(update.fieldMap, { noticeActive: 'notice_active' })
    }
    if (body.noticeDesc !== undefined) {
      Object.assign(update.data, { noticeDesc: body.noticeDesc })
      Object.assign(update.fieldMap, { noticeDesc: 'notice_desc' })
    }
